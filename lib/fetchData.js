...
// Algunos campos de Airtable traen "NAN" o "Invalid Date" como TEXTO literal
// (viene así de una fórmula/lookup roto), no como campo vacío — hay que limpiarlo.
function cleanField(v) {
  if (!v || typeof v !== 'string') return null;
  const trimmed = v.trim();
  if (!trimmed || trimmed.toUpperCase() === 'NAN' || trimmed === 'Invalid Date') return null;
  return trimmed;
}

function priorityRank(p) {
  const s = (p || '').toLowerCase();
  if (s.includes('alta')) return { key: 'alta', order: 0 };
  if (s.includes('media')) return { key: 'media', order: 1 };
  if (s.includes('baja')) return { key: 'baja', order: 2 };
  return { key: 'sd', order: 3 };
}

async function fetchVoltajeData(token) {
  if (!token) throw new Error('Falta AIRTABLE_TOKEN');
  const records = await airtableFetch(token, TABLE_VOLTAJE);
  const hoyISO = new Date().toISOString().slice(0, 10);

  const puntos = records.map(r => {
    const f = r.fields;
    const lat = parseFloat(f['LAT']);
    const long = parseFloat(f['LONG']);
    const prioridadRaw = f['PRIORIDAD'] || '';
    const { key: prioridad, order: prioridadOrden } = priorityRank(prioridadRaw);
    const fechaDeteccion = f['Fecha de Detección'] || null;
    const diasDeteccion = fechaDeteccion
      ? Math.round((new Date(hoyISO + 'T00:00:00') - new Date(fechaDeteccion + 'T00:00:00')) / 86400000)
      : null;

    return {
      serie: cleanField(f['SERIE']) || '',
      modelo: cleanField(f['MODELO']) || '',
      nud: cleanField(f['NUD']) || '',
      establecimiento: cleanField(f['NOMBRE DE ESTABLECIMIENTO']) || 'Sin nombre registrado',
      direccion: cleanField(f['DIRECCION']) || '',
      lat: Number.isFinite(lat) ? lat : null,
      long: Number.isFinite(long) ? long : null,
      contacto: cleanField(f['CONTACTO']),
      tel: cleanField(f['TELEFONO']),
      falla: cleanField(f['FALLA DETECTADA']) || 'Problema de voltaje',
      prioridad,
      prioridadOrden,
      prioridadLabel: prioridadRaw || 'Sin prioridad',
      fechaDeteccion,
      diasDeteccion,
    };
  });

  puntos.sort((a, b) => a.prioridadOrden - b.prioridadOrden);
  return { puntos };
}

function renderVoltajeTemplate(templateHtml, { puntos, hoy }) {
  return templateHtml
    .replace('__HOY__', hoy)
    .replace('__PUNTOS__', JSON.stringify(puntos));
}

module.exports = { fetchBoletinData, renderTemplate, fetchVoltajeData, renderVoltajeTemplate };
